...
def test_prod_message_from_users(review_graph: ReviewGraph) -> None:
    ...
    reviews[0].update_user_to_product(ProductLabel.GOOD, np.log(0.4))
    reviews[0].update_user_to_product(ProductLabel.BAD, np.log(0.6))
    ...
    assert_almost_equal(
        np.exp(
            review_graph.prod_message_from_users(
                reviewers[0], product, ProductLabel.GOOD
            )
        ),
        0.6 * 0.8,
    )
    assert_almost_equal(
        np.exp(
            review_graph.prod_message_from_users(
                reviewers[1], product, ProductLabel.BAD
            )
        ),
        0.6 * 0.2,
    )
    ...
    assert_almost_equal(
        np.exp(
            review_graph.prod_message_from_users(
                None, product, ProductLabel.GOOD
            )
        ),
        0.4 * 0.6 * 0.8,
    )
...
def test_prod_message_from_users_update(review_graph: ReviewGraph) -> None:
    reviewers = [review_graph.new_reviewer(f"reviewer-{i}") for i in range(3)]
    products = [review_graph.new_product(f"product-{i}") for i in range(10)]
    reviews = [
        review_graph.add_review(r, products[0], random.random())
        for r in reviewers
    ]
    for p in products[1:]:
        for r in reviewers:
            review_graph.add_review(r, p, random.random())

    assert_almost_equal(
        review_graph.prod_message_from_users(
            reviewers[0], products[0], ProductLabel.GOOD
        ),
...
def test_prod_message_from_products(review_graph: ReviewGraph) -> None:
    ...
    reviews[0].update_product_to_user(UserLabel.HONEST, np.log(0.4))
    reviews[0].update_product_to_user(UserLabel.FRAUD, np.log(0.6))
    ...
    assert_almost_equal(
        np.exp(
            review_graph.prod_message_from_products(
                reviewer, products[0], UserLabel.HONEST
            )
        ),
        0.6 * 0.8,
    )
    assert_almost_equal(
        np.exp(
            review_graph.prod_message_from_products(
                reviewer, products[1], UserLabel.FRAUD
            )
        ),
        0.6 * 0.2,
    )
    ...
    assert_almost_equal(
        np.exp(
            review_graph.prod_message_from_products(
                reviewer, None, UserLabel.HONEST
            )
        ),
        0.4 * 0.6 * 0.8,
    )
...
def test_prod_message_from_products_update(review_graph: ReviewGraph) -> None:
    reviewers = [review_graph.new_reviewer(f"reviewer-{i}") for i in range(10)]
    products = [review_graph.new_product(f"product-{i}") for i in range(3)]
    reviews = [
        review_graph.add_review(reviewers[0], p, random.random())
        for p in products
    ]
    ...
    assert_almost_equal(
        review_graph.prod_message_from_products(
            reviewers[0], products[0], UserLabel.HONEST
        ),
...
def test_update_user_to_product(review_graph: ReviewGraph) -> None:
    ...
    ans1 = 0.6 * 0.8 * 2.0 * psi(
        ...
        UserLabel.FRAUD, ProductLabel.GOOD, ReviewLabel.PLUS, EPSILON
    )
    assert_almost_equal(
        np.exp(
    ...
    # 2.0: phi of the label (constant)
    # 0.4*0.2: products of other messages to the reviewer with FRAUD.
    ans2 = 0.6 * 0.8 * 2.0 * psi(
        UserLabel.HONEST, ProductLabel.BAD, ReviewLabel.PLUS, EPSILON
    ) + 0.4 * 0.2 * 2.0 * psi(
